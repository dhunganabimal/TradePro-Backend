...
@router.get("/share",status_code=status.HTTP_200_OK, response_model=schemas.My_details_Out)
def my_share_details(
        current_user: models.ShareUsers = Depends(get_current_share_user)):
   return current_user
@router.get("/trade",status_code=status.HTTP_200_OK, response_model=schemas.My_details_Out)
